...
describes what users get. Pick a format with `-o`, or let espn choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
espn <command> -o table   # aligned columns for reading
espn <command> -o jsonl   # one JSON object per line, for piping
espn <command> -o json    # a single JSON array
espn <command> -o csv     # spreadsheet friendly
espn <command> -o tsv     # tab-separated
espn <command> -o url     # just the URL column
espn <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
espn <command> --fields id,title,url
```
...
```bash
espn <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
espn <command>            # a table, because this is a terminal
espn <command> | wc -l    # JSONL, because this is a pipe
```
